...
namespace chipotto
{
	...
	public:
		...
		int GetHeightTexture() const;

		uint16_t GetI() const;

		uint8_t GetIndexRegister(uint8_t index) const;

		uint8_t GetIndexMemoryMap(uint16_t index) const;

		uint8_t GetKeyboardRegIndex() const;

		uint8_t GetDelayTimer() const;

		uint64_t GetDeltaTimerTicks() const;

		uint16_t GetStack() const;
